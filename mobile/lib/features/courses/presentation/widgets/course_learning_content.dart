...
class CourseLearningContent extends StatelessWidget {
  ...
  const CourseLearningContent({super.key, required this.course});
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: _CourseLearningSections(course: course),
    );
  }
}

class _CourseLearningSections extends StatelessWidget {
  final CourseEntity course;
  const _CourseLearningSections({required this.course});

  @override
  Widget build(BuildContext context) {
    final colors = Theme.of(context).colorScheme;

    return BlocConsumer<CourseContentsBloc, CourseContentsState>(
                ...
                ? BlocBuilder<CourseContentsBloc, CourseContentsState>(
                    builder: (context, state) {
                      ...
                      if (state is CourseContentsLoaded) {
                        ...
                        return Column(
                          children: state.course.chapters
                              .map(
...
class _ContinueCard extends StatelessWidget {
  ...
  const _ContinueCard({required this.course, required this.progressSnapshot});
                ...
                MaterialPageRoute(
                  builder: (_) => BlocProvider(
                    ...
                    child: LessonContentPage(
                      initialBlockId: progressSnapshot.currentBlockId ?? 0,
                    ),
                  ),
                ),
              ).then((_) {
